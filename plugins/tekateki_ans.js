...
handler.before = async function (m) {
    ...
    if (/^(me)?nyerah$/i.test(m.text)) {
        await this.sendButton(m.chat, `Jawabannya adalah ${JSON.parse(JSON.stringify(this.tekateki[id][1].jawaban))}`.trim(), '© Origami-Bot', 'TEKA TEKI', '!tekateki', { quoted: m }).then(() => {
    ...
    if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
        ...
        await this.sendButton(m.chat, `*Benar!*\n+${this.tekateki[id][2]} XP`, '© Origami-Bot', 'TEKA TEKI', '!tekateki')
