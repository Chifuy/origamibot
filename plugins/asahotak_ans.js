...
handler.before = async function (m) {
    ...
    if (/^(me)?nyerah$/i.test(m.text)) {
        await this.sendButton(m.chat, `Jawabannya adalah ${JSON.parse(JSON.stringify(this.asahotak[id][1].jawaban))}`.trim(), 'Kok nyerah sih T_T', 'ASAH OTAK', '.asahotak', m).then(() => { delete conn.asahotak[id] })
    }
    ...
    if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
        ...
        await this.sendButton(m.chat, `*Benar!*\n+${this.asahotak[id][2]} XP`.trim(), '', 'ASAH OTAK', '.asahotak', m)
        clearTimeout(this.asahotak[id][3])
        delete this.asahotak[id]
    } else if (similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) m.reply(`*Dikit Lagi!*`)
    else await this.sendButton(m.chat, `*Salah!*`.trim(), '', 'NYERAH', 'nyerah', m)
    // }
    return !0
}
handler.exp = 0
