...
module.exports = {
    ...
    try {
      ...
      try {
        ...
        if (user) {
          ...
          if (!isNumber(user.xpbox)) user.xpbox = 0
          if (!isNumber(user.limit)) user.limit = 10
          if (!isNumber(user.buah)) user.buah = 0
      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          if (xp > 200) m.reply('Ngecheat -_-') // Hehehe
          else m.exp += xp
          ...
          try {
            ...
            if (e) {
              ...
              for (let key of Object.values(global.APIKeys))
                text = text.replace(new RegExp(key, 'g'), '#KEPOLU#')
              m.reply(text)
            }
          } finally {
  ...
  async participantsUpdate({ jid, participants, action }) {
    ...
    switch (action) {
      ...
      case 'demote':
        if (!text) text = (chat.sDemote || this.sdemote || conn.sdemote || '@user ```Sekarang Bukanlah Admin```')
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    group: 'Command kni hanya dapat digunakan didalam Group!',
